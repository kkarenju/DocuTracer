<?php
    ...
    if(isset($_POST['submit'])){
        ...
        {
            $password=md5($pass);
            $qry="SELECT MID,fname,pic FROM member WHERE activation is NULL AND pass='".$password."' AND mail='".$username."' OR phone='".$username."' ";
    		...
    		if($num > 1) {
                    ...
                    while ($row = mysqli_fetch_array($result)) {
                        $username=$pass="";
                        $_SESSION['idd']=$row['MID'];
                        
                        
                        header('Location:includes/header.php');
                        header('Location:includes/header_1.php');
                        
                        header('Location:admin/includes/header.php');
                        header('Location:admin/includes/header_1.php');
                        
                        header('Location:member/includes/header.php');
                        header('Location:member/index.php');
                        echo "<script type='text/javascript'> document.location = 'member/index.php'; </script>";
    			exit();
                    }
                }
