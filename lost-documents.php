<?php 
        ...
        $sql = "SELECT COUNT(*) as num FROM $tableName WHERE status='Lost' "; 
        ...
        $queryy = "SELECT CONCAT(u.fname,' ',u.sname) AS user,d.*,b.name AS bank,b.abbreviation,m.name AS market, s.name AS station,l.date_reported
                    FROM $tableName d
                    LEFT JOIN member u
                        ON d.MID = u.MID
                    LEFT JOIN bank b 
...
<div id="wrap">
    ...
    <?php
        if($rowcount < 1){
            echo '<div class="success"> There are no lost documents. 
